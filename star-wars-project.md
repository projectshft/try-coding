...
## Setup
For simplicity sake, we're going to be using [codepen.io](codepen.io) again. Navigate there now and click "Create", then "New Pen".
...
The two data-structures we care about are `objects`, and `arrays`. As we've seen before, objects are simply key/value pairs in JavaScript. For example, if we were to describe a car, we would want to describe it's attributes. Here is how we might represent a red Ferrari with 500 horsepower with a JavaScript object:
...
### Arrays
Also as we've seen, arrays are lists. We can put anything we want into a list, like strings (text). Here is an array of names:
...
We now have the tools we need to pick apart the Star Wars data. You can now see that the data we got back from the API is an object with 4 properties (`count`, `next`, `previous` and `results`).
...
## Getting Starship
Searching for starships will be very similar to searching for people. We'll just change the URL to look like this:
...
The text after `?search=` is called a "query". We'll change this part of the URL based on what the user enters. In this case, we'll do a search for `death` and the "Death Star" will be our only result.
...
We've now successfully added the Bootstrap CSS to our project.
...
Now that we're getting real data back (though we've hardcoded it and the user cannot yet search), we'll want to append the data to our page.
